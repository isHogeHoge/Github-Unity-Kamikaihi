...
{
    private const float distanceY = 0.51f; // マス目間の距離Y

    // Player停止アニメーション再生(下)
    public void PlayStopAnima(Animator animator)
    {
        animator.Play("PlayerStop_DOWN");
    }
    // Player移動アニメーション再生(下)
    ...
    // Playerキックアニメーション再生(下)
    public void PlayKickAnima(Animator animator)
    {
        animator.Play("PlayerKick_DOWN");
    }
    ...
    public Vector3 SetTargetPos(GameObject obj)
    {
        ...
        if (obj.tag == "Player")
        {
            ...
            // 移動先にあるオブジェクトを全て取得
            RaycastHit2D[] hit2ds = Physics2D.RaycastAll(playerPos, new Vector3(0, -1, 0), distanceY);
        ...
        return obj.transform.position + new Vector3(0, -1, 0) * distanceY;
    }
    ...
    public bool ObstacleExistsToDistination(GameObject col_slime)
    {
        ...
        // キックしたスライムの真下にオブジェクトがないか調べる(rayを+下1マス先へ発射)
        RaycastHit2D[] hit2ds = Physics2D.RaycastAll(col_slimePos, new Vector3(0, -1, 0), distanceY);
        foreach (var i in hit2ds)
        {
